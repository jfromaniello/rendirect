...
describe('rendirects!', function () {
  ...
  describe('when rendirecting', function () {
    ...
    it('should contain the meta tag', function (done) {
      jsdom.env(
        content,
        ...
        function (errors, window) {
          ...
          expect(meta.attr('content').split(';')[0]).to.equal('5');
          expect(meta.attr('content').split(';')[1]).to.equal("URL='http://google.com'");
          done();
        }
      );
    });
  ...
  describe('when rendirecting with no wait', function () {
    var headers, content;
    ...
    beforeEach(function () {
      ...
      res.rendirect({ wait: 0, message: 'Jose is redirecting you', url: 'http://google.com'});
    });
    ...
    it('should contain the meta tag', function (done) {
      jsdom.env(
        ...
        function (errors, window) {
          var meta = window.$('meta');
          expect(window.$('p').length).to.equal(0);
          expect(meta.attr('http-equiv')).to.equal('refresh');
          expect(meta.attr('content').split(';')[0]).to.equal('0');
          expect(meta.attr('content').split(';')[1]).to.equal("URL='http://google.com'");
          done();
        }
      );
